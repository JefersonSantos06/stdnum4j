...
válido, e como ele fica escrito direito* — para 455 tipos de número. Tudo aqui
existe para manter essa única pergunta respondida do mesmo jeito 455 vezes.
...
`format` sejam defaults em vez de 455 cópias escritas à mão.
...
| `stdnum4j-all` | todos acima | Sem código: um `pom` que só agrega, para quem quer os 455 tipos com uma coordenada. E a casa dos testes que precisam de todos os módulos ao mesmo tempo. |
