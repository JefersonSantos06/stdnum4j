...
válido, e como ele fica escrito direito* — para 460 tipos de número. Tudo aqui
existe para manter essa única pergunta respondida do mesmo jeito 460 vezes.
...
`format` sejam defaults em vez de 460 cópias escritas à mão.
...
| `stdnum4j-all` | todos acima | Sem código: um `pom` que só agrega, para quem quer os 460 tipos com uma coordenada. E a casa dos testes que precisam de todos os módulos ao mesmo tempo. |
