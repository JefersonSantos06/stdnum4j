...
válido, e como ele fica escrito direito* — para 458 tipos de número. Tudo aqui
existe para manter essa única pergunta respondida do mesmo jeito 458 vezes.
...
postal britânico tem oito. Os 130 tipos que têm máscara a devolvem; os demais
...
`format` sejam defaults em vez de 458 cópias escritas à mão.
...
| `stdnum4j-all` | todos acima | Sem código: um `pom` que só agrega, para quem quer os 458 tipos com uma coordenada. E a casa dos testes que precisam de todos os módulos ao mesmo tempo. |
...
brasileiros puxa `stdnum4j-br` e ganha `stdnum4j-core`, não 131 tipos europeus.
