...
válido, e como ele fica escrito direito* — para 464 tipos de número. Tudo aqui
existe para manter essa única pergunta respondida do mesmo jeito 464 vezes.
...
postal britânico tem oito. Os 133 tipos que têm máscara a devolvem; os demais
...
`format` sejam defaults em vez de 464 cópias escritas à mão.
...
| `stdnum4j-all` | todos acima | Sem código: um `pom` que só agrega, para quem quer os 464 tipos com uma coordenada. E a casa dos testes que precisam de todos os módulos ao mesmo tempo. |
